 ...
 * ✅ action=create:
 *    - Computes intended filename from Deal Name + template suffix
 *    - Checks destination folder for that EXACT filename
...
async function createFeeSheetFromTemplate({ dealId, hubspotToken, createdBy }) {
  ...
  // 2) Get template item name (to preserve suffix rule)
  ...
  // Helper: find file by name in destination folder
  ...
  if (existingItem?.id) {
    ...
    return {
      message: "Fee Sheet already exists — opening it now.",
  ...
  for (let i = 0; i < 15; i++) {
    ...
    // if a file with this name already exists, try the next suffix
    if (txt.includes('"code":"nameAlreadyExists"')) continue;
  ...
  if (!createdItem?.id) {
    ...
    return {
      message:
        "Your Fee Sheet is being prepared. Refresh the deal in about a minute to confirm it's ready.",
      ...(await buildFlatCardMetaFast(dealId, hubspotToken)),
    };
  }
...
app.all("/api/fee-sheet", async (req, res) => {
  try {
    ...
    if (!dealId) {
      return res.status(400).json({ message: "Missing deal ID." });
    }
    ...
    if (action === "refresh") {
      ...
      let amountNote = "Synced. Deal amount unchanged.";
      ...
      if (j7.amount !== null) {
        ...
        if (storedAmount === attemptedAmount) {
          amountNote = `Synced — deal amount updated to ${attemptedAmount}.`;
        } else {
          amountNote = `Synced, but the deal amount may not have updated. HubSpot shows ${
            storedAmount || "(blank)"
          } — check the deal and sync again if needed.`;
        }
      } else {
        await hubspotPatchDeal(dealId, HUBSPOT_TOKEN, {
          fee_sheet_last_synced_at: toIsoNow(),
    ...
    if (action === "set-ready" || action === "setReady") {
      ...
      return res.json({
        message: next
          ? "Fee Sheet marked ready for proposal."
          : "Fee Sheet reopened for editing.",
